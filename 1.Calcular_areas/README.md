...
Este diretório contém um Jupyter Notebook (`calculo_de_areas.ipynb`) desenvolvido para automatizar o processamento e o cálculo de áreas de arquivos geoespaciais. O script utiliza a biblioteca `geopandas` para ler os dados, manipular as geometrias e gerar novos arquivos com métricas de área atualizadas.
...
* **Suporte a Múltiplos Formatos:** Aceita arquivos `.shp` (Shapefile) e `.kml` / `.kmz` como dados de entrada.
...
2.  **Configuração de Diretórios:** Vá até a célula intitulada "definir caminhos fixos" e altere as variáveis `in_dir` e `out_dir` para que apontem para os caminhos absolutos das pastas de entrada e saída de arquivos no seu sistema.
    * *Alternativa interativa:* Você pode descomentar o código na seção "copiar caminhos no prompt" para inserir os caminhos dinamicamente durante a execução via função `input()`.
...
4.  Execute as células sequencialmente para importar os pacotes e declarar as funções de trabalho (`area_calc`, `unify_geometries` e `area_files_list`).
5.  **Execução Final:** Na última seção ("Aplicação das funções"), descomente a linha `# area_files_list(in_dir,out_dir,projecao)` e execute a célula para iniciar o processamento em lote. O log no console informará o andamento arquivo por arquivo e salvará os novos `.shp` na pasta de saída.

---

### Funcionamento Interno (Visão Técnica)

A lógica central deste código é estruturada em torno de manipulações de `GeoDataFrames` utilizando o `geopandas`. O fluxo principal opera da seguinte maneira:

1.  A função central `area_files_list()` itera sobre a lista de arquivos encontrados na pasta fonte de dados.
2.  Cada arquivo é lido e instanciado como um objeto `gpd.GeoDataFrame` através da instrução `gpd.read_file()`.
3.  Caso a flag booleana de unificação esteja ativa, o código isola a primeira linha (`gdf[0:1].copy()`) e sobrescreve sua coluna geométrica pela união massiva de todos os dados do shape usando `gdf.geometry.union_all()`.
4.  Durante o cálculo na função `area_calc`, o dado sofre o processo analítico de reprojeção (`to_crs()`), seguido pelo cálculo algébrico `gdf.area / 10000` para registrar o quantitativo em hectares.
5.  A tabela do `GeoDataFrame` é filtrada mantendo estritamente os campos `["area_ha", "area_max", "area_min", "geometry"]`, finalizando com o dump do arquivo no disco com a instrução de saída `.to_file()`.